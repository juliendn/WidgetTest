 ...
 */
public class SimpleWiktionaryHelper
{
	private static final String TAG = "SimpleWikiHelper";

	/**
	 * Regular expression that splits "Word of the day" entry into word name, word type, and the first description bullet point.
	 */
	public static final String WORD_OF_DAY_REGEX = "(?s)\\{\\{wotd\\|(.+?)\\|(.+?)\\|([^#\\|]+).*?\\}\\}";

	/**
	 * Partial URL to use when requesting the detailed entry for a specific Wiktionary page. Use {@link String#format(String, Object...)} to insert the desired page title after escaping it as needed.
	 */
	...
	/**
	 * Partial URL to append to {@link #WIKTIONARY_PAGE} when you want to expand any templates found on the requested page. This is useful when browsing full entries, but may use more network bandwidth.
	 */
	...
	/**
	 * Shared buffer used by {@link #getUrlContent(String)} when reading results from an API request.
	 */
	...
	/**
	 * Thrown when there were problems parsing the response to an API call, either because the response was empty, or it was malformed.
	 */
